...
type SentFile = { id: string; name: string; size: number; category: string };
...
export function AdditionalUpload({
  ...
  const [category, setCategory] = useState("financial_history");
  ...
  async function handle(list: FileList | null) {
    ...
    for (const original of selected) {
      ...
      if (res.status === 201 && res.body.file) {
        const f = res.body.file as { id: string; name: string; size: number; category: string };
        setFiles((current) => [...current, { id: f.id, name: f.name, size: f.size, category: f.category }]);
  ...
  const items: FileRowItem[] = [
    ...files.map((f) => ({ key: f.id, name: f.name, size: f.size, meta: categoryLabel(f.category) })),
  ...
  return (
    <section className="mt-6 rounded-2xl border border-line bg-surface p-5 shadow-soft">
      ...
      />
      <Button variant="secondary" className="mt-5 w-full" onClick={() => input.current?.click()}>
        <IconPlus size={18} /> Enviar arquivo
      </Button>
      <p className="mt-2 text-xs text-muted">PDF, CSV, XLSX, JPG ou PNG, até {maxUploadMb} MB por arquivo.</p>
      {items.length > 0 && (
